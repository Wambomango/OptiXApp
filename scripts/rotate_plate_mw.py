...
renderer = mwir.ManyWorldsRenderer()

sender = mwir.Antenna((0, 0, 0), (0, 0, 0), (0.2, 0.2), 1E9)
...
many_worlds = mwir.ManyWorlds([plate_distance - 1, -1, -1], [plate_distance + 1, 1, 1], 0.01, 1)
...
for i in range(n_angles):
    ...
    E_rx = renderer.Render(scene, many_worlds)
    rcs[i] = 10 * torch.log10(4 * torch.pi * plate_distance**2 * torch.linalg.vector_norm(E_rx, dim = 2)**2)



# print(many_worlds.GetOccupancy().shape)
# plt.imshow(np.log(0.1 + many_worlds.GetOccupancy().detach().cpu().numpy()[:,:,100]))
# plt.show()




# print(many_worlds.GetOccupancy())

plt.plot(angles * 180 / torch.pi, rcs.detach().numpy())
