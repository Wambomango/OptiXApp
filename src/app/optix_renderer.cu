...
void OptiXRenderer::Render(Camera &camera) 
{
    ...
    glm::vec3 camera_w = -glm::vec3(std::sin(azimuth) * std::cos(elevation), std::sin(elevation), std::cos(azimuth) * std::cos(elevation)) * near_plane;
    glm::vec3 camera_u = glm::normalize(glm::cross(camera_w, glm::vec3(0.0f, 1.0f, 0.0f))) * near_plane * std::atan(fov * 0.5f);
    glm::vec3 camera_v = glm::normalize(glm::cross(camera_u, camera_w)) * near_plane * std::atan(fov * 0.5f) * aspect_ratio;

    Params p;
    p.image = reinterpret_cast<unsigned int *>(image);
    p.image_width = width;
...
void OptiXRenderer::BuildGAS(Scene &scene) 
{
    ...
    const std::array<float3, 3> vertices = {{{-0.5f, -0.5f, 0.0f},
                                                {0.5f, -0.5f, 0.0f},
                                                {0.0f, 0.5f, 0.0f}}};
