...
void OptiXRenderer::Render(Camera &camera) 
{
    ...
    glm::vec3 camera_u = glm::normalize(glm::vec3(std::cos(azimuth) * std::cos(elevation), std::sin(azimuth) * std::cos(elevation), std::sin(elevation))) * near_plane;
    glm::vec3 camera_v = glm::normalize(glm::cross(camera_u, glm::vec3(0.0f, 0.0f, 1.0f))) * near_plane * std::tan(fov * 0.5f);
    glm::vec3 camera_w = glm::normalize(glm::cross(camera_v, camera_u)) * near_plane * std::tan(fov * 0.5f) * aspect_ratio;
