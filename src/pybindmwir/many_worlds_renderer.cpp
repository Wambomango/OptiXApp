...
torch::Tensor ManyWorldsRenderer::Backward(std::shared_ptr<Scene> scene, std::shared_ptr<ManyWorlds> many_worlds, torch::Tensor grad_output, std::optional<torch::Tensor> grad_opacity, std::optional<int> seed)
{
    if (!mwir_renderer_)
    {
        throw std::runtime_error("ManyWorldsRenderer ownership has been transferred.");
    ...
    return mwir_renderer_->Backward(*scene->mwir_scene_, *many_worlds->mwir_many_worlds_, grad_output, grad_opacity, seed);
}
